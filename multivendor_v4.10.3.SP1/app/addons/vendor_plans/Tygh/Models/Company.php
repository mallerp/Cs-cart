...
class Company extends AModel
{
        ...
        if ($plan = $this->plan) {
            if ($price = floatval($plan->price)) {
                $payout_id = VendorPayouts::instance()->update(array(
                    ...
                    'comments'       => __("addons.vendor_plans.payout_for_the_plan", array('[plan]' => $plan->plan))
                ));
                ...
                $mailer->send(array(
                    'to' => $this->getEmail('company_support_department') ?: 'company_support_department',
    ...
    {
        $payout_exists = db_get_row(
            "SELECT * FROM ?:vendor_payouts WHERE company_id = ?i AND plan_id = ?i LIMIT 1",
            $this->company_id, $this->plan_id
        );
        if (!$payout_exists) {
            $this->payment();
        }
